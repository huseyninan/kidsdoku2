    ...
    func puzzles(for size: Int, difficulty: PuzzleDifficulty, themeType: GameThemeType) -> [PremadePuzzle] {
        ...
        return basePuzzles.map { puzzle in
            ...
            return PremadePuzzle(
                id: puzzle.id, number: puzzle.number,
                size: puzzle.size,
...
private func puzzle(
    ...
    // Generate stable ID in format "size-difficulty-number"
    let stableId = "\(size)-\(difficulty.rawValue)-\(number)"
